...
const STORE_BASE_URL = "https://shop.aeon.com/netsuper/01050000004000/";

// イオンスタイル倉敷もイオンスタイル岡山青江と同様、カテゴリページのURLが店舗固有で未特定のため、
// キーワード検索方式で取得する
async function main() {
  const store = await getStoreByName("イオンネットスーパー イオンスタイル倉敷");
  if (!store) throw new Error("store not found: イオンネットスーパー イオンスタイル倉敷");
